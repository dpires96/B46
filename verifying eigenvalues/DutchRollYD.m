...
% Symmetric: phugoid 0:56 (index around 33460)  Short period 0:59 (35735)
% Asymmetric: Aper. Roll 1:01 (sometime before 36510) Dutch Roll 1:03  Dutch Roll YD 1:04  Spiral 1:05

fd =  load('FTISxprt-20200311_flight2');
asym = 1; %0 if symmetric, 1 if assymetric

startindex = 38800; %(Second-9)*10
refindex = 38800; %same thing
...
CLa    = 5.958761;            % Slope of CL-alpha curve [1/rad]

% Longitudinal stability
Cma    = -1.037;            % longitudinal stabilty [1/rad]
Cmde   = -1.886;            % elevator effectiveness [1/rad]
...
if asym == 1
    ...
    % From body axis to stab axis
    roll0 = fd.flightdata.Ahrs1_Roll.data(refindex)*pi/180;
    rr0 = fd.flightdata.Ahrs1_bRollRate.data(refindex)*pi/180;
    yr0 = fd.flightdata.Ahrs1_bYawRate.data(refindex)*pi/180;
    
    

    Tfinal = 20; %Seconds
    das = fd.flightdata.delta_a.data;
    drs = fd.flightdata.delta_r.data;
    da = das(startindex:startindex+Tfinal*10);
    dr = drs(startindex:startindex+Tfinal*10);
    u = [da dr];
    x0 = [ 0; 0; 0; 0]; %beta, roll angle, roll rate, yaw rate
    t = 0:0.1:Tfinal;
    t = transpose(t);
    y = lsim(sysa, u*pi/180, t, x0);

    truerolls = fd.flightdata.Ahrs1_Roll.data(startindex:startindex+Tfinal*10)*pi/180-roll0;
    truerrs = fd.flightdata.Ahrs1_bRollRate.data(startindex:startindex+Tfinal*10)*pi/180-rr0;
    trueyrs = fd.flightdata.Ahrs1_bYawRate.data(startindex:startindex+Tfinal*10)*pi/180-yr0;

%     plot(t,da);
%     hold on
%     plot(t,dr);
%     hold on
    plot(t,y(:,2)*180/pi);%For degrees multiply by 180/pi
    hold on
    plot(t,truerolls*180/pi);%For degrees multiply by 180/pi
    hold off
end